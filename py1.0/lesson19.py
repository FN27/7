...
def guess(event):
    ...
    if number=='':
        ...
        if attempts >0:
            ...
            game_plan['user_health']='Можешь сыграть ещё раз'
            gues_button.configure(state=tk.DISABLED)
            return_button.configure(state=tk.NORMAL)
window=tk.Tk()
...
gues_button=tk.Button(window,width=17,text='Проверить',command=lambda e='<Return>':guess(e))
gues_button.place(x=10,y=90)
return_button=tk.Button(window,text='Играть снова',state=tk.DISABLED,width=17, command=reset)
