...
TILE_SCALING = 0.5
CHARACTER_SCALING = TILE_SCALING * 1.7
...
class EntityDoll(arcade.Sprite):
    def __init__(self, name_folder, name_file):
        super().__init__()
        ...
        main_path = f"C:/Users/User/Desktop/Федя/Assets/platformer/{name_folder}/{name_file}"
...
class AllieDoll(EntityDoll):
    def __init__(self, name_folder, name_file):
        # Setup parent class
        super().__init__(name_folder, name_file)


class VillagerSeller(AllieDoll):
    def __init__(self):
        super().__init__("Sprite_Villager", "Villager_idol_0.png")
...
class MyGame(arcade.Window):
    ...
    def setup(self):
        ...
        allies_layer = self.tile_map.object_lists[LAYER_NAME_ALLIES]

        for my_object in allies_layer:
            cartesian = self.tile_map.get_cartesian(
                my_object.shape[0], my_object.shape[1]
            )
            allie_type = my_object.properties["type"]
            if allie_type == "VillagerSeller":
                alie = VillagerSeller()
            else:
                raise Exception(f"Unknown alie type {allie_type}.")
            alie.center_x = math.floor(
                cartesian[0] * TILE_SCALING * self.tile_map.tile_width
            )
            alie.center_y = math.floor(
                (cartesian[1] + 1) * (self.tile_map.tile_height * TILE_SCALING)
            )
            self.scene.add_sprite(LAYER_NAME_ALLIES, alie)
